...
def iter_package_files(path: Path) -> tuple[list[dict[str, Any]], list[TextFile]]:
    ...
    def add_file(name: str, data: bytes) -> None:
        ...
        digest = sha256(data)
        kind = "text" if suffix in TEXT_EXTENSIONS else "binary-or-unknown"
        inventory.append({"path": name, "size": len(data), "sha256": digest, "kind": kind})
        if suffix in TEXT_EXTENSIONS and len(data) <= 10_000_000:
...
def parse_candidates(text_files: list[TextFile], expected_length: int | None) -> list[Candidate]:
    ...
    for file in text_files:
        ...
        if not parsed and suffix in {".txt", ".out", ".log", ".md", ".csv", ".tsv"}:
...
def interpretation_text(
    ...
    lines = [
        "# PepMLM Result Interpretation",
        "",
        "## One-line Conclusion",
    ]
    if rows:
        lines.append(
            f"识别到 {len(rows)} 条去重 PepMLM 候选肽；这些结果只能作为生成器优先级，不能直接证明结合、降解或机制。"
        )
    else:
        lines.append("已完成原始结果审计，但没有识别到可解析的候选肽表；当前不能给出候选排序。")
    lines.extend(
        [
            "",
            "## Raw Evidence",
